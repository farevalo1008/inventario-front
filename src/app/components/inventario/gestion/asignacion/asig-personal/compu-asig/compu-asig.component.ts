...
@Component({
  selector: 'app-compu-asig',
  templateUrl: './compu-asig.component.html',
  styleUrls: ['./compu-asig.component.css'],
  providers: [RegistroartService]
})
export class CompuAsigComponent implements OnInit {
 ...
 for (var i = 0; i < listaCodigo.length; i++) {   
    for (var j = 0; j < this.uso.length; j++) {   
        ...
        if(listaCodigo[i].id_articulo == this.uso[j].id_articulo){
          console.log("linea 281",listaCodigo[i].id_articulo);
